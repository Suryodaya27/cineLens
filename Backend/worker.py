...
def download_image(url: str, save_path: Path) -> bool:
    ...
    import requests
    try:
        resp = requests.get(str(url), timeout=30, stream=True)
...
def main():
    r = get_redis()
    print(f"🔧 Worker started — listening on queue: {JOB_QUEUE}")
    ...
    while True:
        ...
        try:
            ...
            print(f"   Movie: {params.get('movie_name')}")
            print(f"   Vision: {'enabled' if params.get('enable_vision') else 'disabled'}")
            print(f"{'='*70}")
